  ...
  return (
    <article className="border-b md:border md:rounded-xl bg-card w-full mb-4 py-2">
      ...
      <div className="flex items-center justify-between p-3">
        <div className="flex items-center gap-3">
          <Avatar className="w-8 h-8 mt-1">
            <AvatarImage src={post.user.avatar} alt={post.user.name}></AvatarImage>
            <AvatarFallback>{post.user.name[0]}</AvatarFallback>
          </Avatar>
          <div className="flex flex-col">
            <span className="font-semibold text-sm text-left">{post.user.username}</span>
            {post.location && (
      ...
      <div className="pb-0 flex items-center gap-1">
        <Button variant="ghost" size='icon' className='hover:text-red-500 [&_svg]:size-6'>
          <Heart />
        </Button>
        ...
        <div className="p-3 pt-1 space-y-1">
          <div className="font-semibold text-sm text-left">{post.likes} curtidas</div>
